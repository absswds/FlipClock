...
fun calculateFlipDurationLayout(
    ...
    val groupGapRatio = 0.06f
    // Each separator has a gap before and after, so 2 × groupGapRatio
    val weightedGlyphs = safeDigitCount + separatorCount * (separatorWeight + 2 * groupGapRatio)
...
fun FlipDurationDisplay(
    ...
    BoxWithConstraints(
        ...
        val groupGap = glyphWidth * 0.06f

        Row(verticalAlignment = Alignment.CenterVertically) {
            parts.forEachIndexed { index, part ->
                // Add spacing before separators (gaps between flip cards and colons)
                if (index > 0 && part is FlipTextPart.Separator) {
